...
import { AppSettings, Occasion, OutfitCandidate, SuggestionFeedbackKind, WardrobeCategory, WardrobeItem, WearLog, occasionOptions } from '../../types';
...
export type TodayTabProps = {
  ...
  onFeedback: (candidate: OutfitCandidate, kind: SuggestionFeedbackKind) => void;
  formatDaysAgo: (days: number) => string;
};

export function TodayTab(props: TodayTabProps) {
  const { styles, onboarding, cityDraft, setCityDraft, refreshWeather, weatherLoading, syncCalendar, calendarLoading, temperatureLabel, settings, calendarEventTitle, resolvedOccasion, setSettings, suggestions, wearLogs, missing, wardrobe, swap, wearCandidate, onFeedback, formatDaysAgo } = props;
  return <>
